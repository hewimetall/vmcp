...
/// 1:1 (prefixed `{server}__{tool}`) instead of the GraphQL semantic layer.
/// The main `/mcp` endpoint keeps serving GraphQL — both run in the same
...
impl Default for UpstreamConfig {
    fn default() -> Self {
        Self { spawn_timeout_ms: 30_000, call_timeout_ms: 60_000 }
    }
}
...
impl Settings {
    ...
    /// Validate cross-field invariants. Call after `load`.
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        if !self.mcp_path.starts_with('/') {
            return Err(ConfigError::Validation("mcp_path must start with '/'".into()));
        }
...
/// Load settings from `path` (TOML) + env-var overrides (`VMCP_*`).
///
