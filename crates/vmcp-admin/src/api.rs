//! Read-only JSON API for the admin panel.
//!
//! All routes return `application/json`. Templates use these endpoints via
//! Grid.js (`server: { url: ... }`) and ad-hoc fetch calls in
//! `static/sessions.js`. The same data drives both HTML and programmatic
//! clients (curl, scripts, exporters).
...
pub fn routes() -> Router<AdminState> {
    Router::new()
        ...
        .route(
            "/api/skills/:name",
            get(get_skill).put(update_skill).delete(delete_skill_handler),
        )
        .route("/api/skills/:name/generate", post(render_skill_handler))
        .route("/api/notifications", get(get_notifications))
        // Sessions API — sibling agent C.
        .route("/api/sessions", get(list_sessions))
        .route("/api/sessions/:client_id", get(get_session_detail))
...
async fn get_schema_sdl(State(s): State<AdminState>) -> impl IntoResponse {
    ...
    ([(axum::http::header::CONTENT_TYPE, "text/plain; charset=utf-8")], sdl)
}
...
async fn render_skill_handler(
    ...
    let rendered = render_skill_template(&skill, &req.args)
...
async fn list_sessions(State(s): State<AdminState>) -> Json<Value> {
    ...
    let disk = s.recorder.list_all_clients_with_meta().await.unwrap_or_default();

    // Index by client_id so registered/live/disk views can merge cleanly.
    ...
    // Merge live sessions — promote `pre_registered` → `active` when a live
    // session attaches.
    for ss in &live {
    ...
    // Merge disk metas (historical) — skip session_ids already covered by
    // the live snapshot to avoid duplicate rows.
    for (cid, metas) in &disk {
        ...
        for m in metas {
            // If the live snapshot already covers this session_id we keep
            // the live row (it carries fresher counters) but still merge
            // the disk-derived `upstream` tag onto it — the live row has
            // no upstream until the recorder flushes meta, and we want
            // the endpoint badge to appear in the admin UI as early as
            // possible (compare-demo).
            let live_idx = entry
                .get("sessions")
...
async fn get_session_detail(
    ...
    let live: Vec<_> = s
        ...
        .collect();
    let disk = s.recorder.list_client_sessions(&client_id).await.unwrap_or_default();
...
async fn dump_handler(
    ...
    if !dir.is_dir() {
        // Either no client at all (404) or client exists but no dump yet (200 empty).
        let has_client = s
            .auth_state
            .list_clients()
            .iter()
    ...
    // Read & merge — the recorder writes one McpExchange per jsonl line. The
    // first line is a SessionMeta header, which won't deserialize as
    // McpExchange and is silently skipped.
    ...
    Ok(Json(json!({ "exchanges": filtered, "next_seq_hint": next })))
}
...
> {
    ...
    let stream = BroadcastStream::new(receiver).map(|res| match res {
        Ok(ex) => Ok(Event::default().json_data(&*ex).unwrap_or_default()),
    ...
    let boxed: std::pin::Pin<
        Box<
            dyn futures::Stream<Item = Result<Event, std::convert::Infallible>>
                + Send,
        >,
    > = Box::pin(stream);
...
async fn dump_download(
    ...
    let path = s.recorder.root.join(&client_id).join(format!("{session_id}.jsonl"));
...
mod validation_tests {
    use super::{validate_client_id, validate_session_id};
