...
pub fn build_router(state: AuthState) -> Router {
    Router::new()
        ...
        // RFC 9728 protected-resource metadata. Two routes, same handler:
        //   - bare /.well-known/oauth-protected-resource
        //   - path-scoped /.well-known/oauth-protected-resource/mcp
        // Our own middleware emits the bare URL in the WWW-Authenticate
        // resource_metadata= challenge, and Anthropic's MCP connector
        // fetches whatever the challenge points to — so the bare route is
        // mandatory or Claude's first probe 404s and aborts the OAuth flow
        // ("ofid_..." reference). The /mcp suffix is kept for compatibility
        // with clients that follow the MCP authorization profile draft.
        .route(
...
async fn rs_metadata(State(s): State<AuthState>) -> Json<ProtectedResourceMetadata> {
    Json(ProtectedResourceMetadata {
        resource: s.resource_audience.clone(),
...
async fn token_endpoint(
    ...
    // Resource indicator: enforce explicit audience binding.
    let audience = resource.unwrap_or_else(|| s.resource_audience.clone());
    if audience != s.resource_audience {
        return Err(AuthError::BadRequest(
