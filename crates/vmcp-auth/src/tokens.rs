...
/// the provided `expected_audience` exactly (one MCP endpoint per gateway).
pub fn verify_access_token(
    ...
    let data =
        decode::<AccessTokenClaims>(bearer, &key, &validation).context("jwt verify")?;
    Ok(data.claims)
}
...
mod tests {
    ...
    fn issue_and_verify_round_trip() {
        ...
        let (tok, _) =
            issue_access_token(&mgr, "https://iss", "https://iss/mcp", "client-x", "mcp:use", 60)
                .unwrap();
