...
**Язык:** [English](../skills.md) | Русский
...
В gate по покрытию (llvm-cov) для `vmcp-server` входят: `skills.rs`, `prompt_catalog.rs`, `prompt_proxy.rs` (helpers) и `graphql_inject.rs` — вместе с `sessions.rs` и `tasks.rs`. Порог — **96% покрытия строк**.
