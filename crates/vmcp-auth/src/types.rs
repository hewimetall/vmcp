...
pub struct ClientInfo {
    pub client_id: String,
    pub redirect_uris: Vec<String>,
    /// RFC 7591 `client_name` from the registering host (often always `"Cursor"`).
    pub client_name: Option<String>,
    /// Operator-facing unique label for the admin Sessions UI. Auto-generated
    /// on DCR (`cursor`, `cursor-2`, …) and editable via admin PATCH.
    pub name: String,
    pub grant_types: Vec<String>,
    pub response_types: Vec<String>,
    pub scope: Option<String>,
...
/// Validate an operator-facing DCR display name: `^[a-z0-9_-]{1,64}$`.
pub fn valid_client_display_name(name: &str) -> bool {
    let b = name.as_bytes();
    (1..=64).contains(&b.len())
        && b.iter()
...
/// Slugify RFC `client_name` into a base for auto-generated unique `name`.
pub fn slugify_client_base(client_name: Option<&str>) -> String {
    let raw = client_name.unwrap_or("client");
...
/// Pick the next free name given a base slug and the set of already-used names.
pub fn next_unique_name(existing: &std::collections::HashSet<String>, base: &str) -> String {
    ...
    unreachable!("u32 counter exhausted for name allocation")
}
