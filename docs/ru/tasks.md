...
| Line coverage gate (≥96%, включает `tasks` + `sessions` + `skills` + prompt agg) | `cargo llvm-cov -p vmcp-server --lib --fail-under-lines 96 --ignore-filename-regex '(^|/)(otel_file|proxy|lib|recorder)\.rs$'` |
