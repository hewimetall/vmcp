...
— см. [`docs/ru/tasks.md`](docs/ru/tasks.md). Опциональный `[mcp].latest`
рекламирует протокол `2026-07-28` рядом с legacy (`2025-11-25`); по
умолчанию выключен — см. [`docs/ru/mcp-2026-07-28.md`](docs/ru/mcp-2026-07-28.md). Опциональные Python benchmarks находятся
...
- **Demo:** [`demo/README.ru.md`](demo/README.ru.md) + конфиг [`demo/vmcp.toml`](demo/vmcp.toml).

### Сборка, тесты, линт

Использование описано в `README.ru.md`. Стандартные команды из корня репозитория:
...
| Coverage CI | `.github/workflows/ci.yml` — CI проверяет соблюдение указанных выше порогов покрытия. |
| Sessions | JSON в `[recorder].sessions_dir` (`.registry/` + dumps) — см. [`docs/ru/sessions.md`](docs/ru/sessions.md) |
| Upstream registry | `registry.json` + sidecars + skills/prompts — см. [`docs/ru/upstreams.md`](docs/ru/upstreams.md) |
| Skills / prompts | YAML в `skills_dir` + upstream через `[proxy]` — см. [`docs/ru/skills.md`](docs/ru/skills.md) |
| DCR clients | SQLite в `[auth].clients_db_path` (сохраняется после restart) — см. [`docs/ru/authentication.md`](docs/ru/authentication.md#dcr-clients-переживают-restart) |
...
- **bench/** — см. [`docs/ru/bench.md`](docs/ru/bench.md).
