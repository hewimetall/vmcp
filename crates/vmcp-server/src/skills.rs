...
fn parse_skill_file(path: &Path) -> Result<Skill> {
    ...
    let skill: Skill = serde_yaml::from_str(&text).with_context(|| format!("parse {path:?}"))?;
    if skill.name.is_empty() {
        return Err(anyhow!("skill at {path:?} has empty name"));
    }
    if !name_re().is_match(&skill.name) {
        return Err(anyhow!(
            "skill {} has invalid name (must match ^[a-z0-9_-]{{1,64}}$)",
            skill.name
        ));
    }
    // Reserved: GraphQL / `/mcp-proxy` uses `{server}__{name}` for upstream
    // prompts. A local skill with `__` would collide with that namespace and
    // make `getPrompt` / `prompts/get` ambiguous.
    if skill.name.contains("__") {
        return Err(anyhow!(
            "skill {} must not contain `__` (reserved for upstream prompt names)",
            skill.name
        ));
    }
...
pub fn save_skill(dir: &std::path::Path, skill: &Skill) -> anyhow::Result<()> {
    ...
    if skill.name.contains("__") {
        anyhow::bail!("invalid name: must not contain `__` (reserved for upstream prompts)");
    }
...
"#,
    ...
    #[test]
    fn load_skips_non_yaml_subdir_and_duplicates() {
        let nanos = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("vmcp-skills-skip-{nanos}"));
        fs::create_dir_all(&dir).unwrap();
        fs::create_dir_all(dir.join("nested")).unwrap();
        fs::write(dir.join("notes.txt"), b"not yaml").unwrap();
        fs::write(
            dir.join("a.yaml"),
            r#"
name: twin
...
"#,
        ...
        assert_eq!(skills[1].name, "twin");
        let _ = fs::remove_dir_all(&dir);
    }

    #[test]
    fn load_skips_malformed_empty_name_and_empty_description() {
        let nanos = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("vmcp-skills-bad-{nanos}"));
        fs::create_dir_all(&dir).unwrap();
        fs::write(dir.join("bad.yaml"), b"not: [valid: yaml: {{{").unwrap();
        fs::write(
            dir.join("empty-name.yaml"),
            r#"
name: ""
description: x
template: "t"
...
mod save_delete_tests {
    ...
    #[test]
    fn save_rejects_name_with_double_underscore() {
        let dir = fresh_dir("bad-dunder");
        let mut s = valid_skill("greet");
        s.name = "tavily__research".into();
        let err = save_skill(&dir, &s).unwrap_err();
        assert!(
            err.to_string().contains("must not contain `__`"),
            "got: {err}"
        );
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    #[test]
    fn load_skips_skill_with_double_underscore_name() {
        let dir = fresh_dir("load-dunder");
        fs::create_dir_all(&dir).unwrap();
        fs::write(
            dir.join("bad.yaml"),
            "name: foo__bar\ndescription: collide\ntemplate: x\n",
        ...
        .unwrap();
        let loaded = load_skills(&dir).expect("load ok");
        assert!(loaded.is_empty(), "dunder names must be skipped");
        let _ = fs::remove_dir_all(&dir);
    }
