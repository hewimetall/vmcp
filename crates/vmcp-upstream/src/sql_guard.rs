            ...
            TableFactor::Table {
                ...
                if let Some(ident) = function_last_ident(name) {
                    if is_forbidden(ident) {
                        return ControlFlow::Break(GuardError::ForbiddenFunction(ident.to_string()));
                    }
                }
            }
...
fn function_last_ident(name: &sqlparser::ast::ObjectName) -> Option<&str> {
    name.0.last().and_then(|p| p.as_ident()).map(|i| i.value.as_str())
}
...
mod tests {
    ...
    fn blocks_pg_read_file_in_projection() {
        assert_eq!(err_fn("SELECT pg_read_file('postgresql.conf')"), "pg_read_file");
    }
    ...
    fn blocks_pg_read_file_in_derived_table() {
        assert_eq!(
            err_fn(
                "SELECT * FROM (VALUES (1)) v(c) WHERE c::text = pg_read_file('/etc/passwd')"
            ),
            "pg_read_file"
        );
    }
    ...
    fn blocks_pg_read_file_in_join_on() {
        assert_eq!(
            err_fn(
                "SELECT * FROM a JOIN b ON a.id = b.id AND pg_read_file('x') IS NOT NULL"
            ),
            "pg_read_file"
        );
    }
    ...
    fn blocks_pg_stat_file() {
        assert_eq!(err_fn("SELECT pg_stat_file('postgresql.conf')"), "pg_stat_file");
    }
