...
pub fn build_schema(
    ...
    let json_scalar = Scalar::new("JSON")
        .description("Arbitrary JSON. Passed verbatim to/from upstream tools.");
    ...
    let server_obj = Object::new("Server")
        .description("Connected upstream MCP server. Operator-curated description is the cheapest \
    ...
    let search_hit_obj = Object::new("SearchHit")
        .description("One ranked match from `search(q)`. The agent uses `server` + `tool` + \
                    ...
                    FieldFuture::new(async move {
                        ...
                        for (server, tools) in pool.all_resolved() {
                            for t in tools {
                                ...
                                .to_lowercase();
                                let score =
                                    tokens.iter().filter(|tk| hay.contains(*tk)).count();
                                if score > 0 {
                                    hits.push((
                                        score,
                                        SearchHitNode {
                        ...
                        hits.sort_by(|a, b| b.0.cmp(&a.0).then_with(|| a.1.server.cmp(&b.1.server)));
                        Ok(Some(FieldValue::list(
                            hits.into_iter()
                                .map(|(_, n)| FieldValue::owned_any(n)),
                        )))
                    })
                },
            )
...
mod tests {
    ...
    fn build_field_args_emits_inverse_map_for_snake_props() {
        ...
        assert_eq!(map.get("sourceTimezone").map(String::as_str), Some("source_timezone"));
        assert_eq!(map.get("targetTimezone").map(String::as_str), Some("target_timezone"));
    ...
    fn build_field_args_identity_for_already_camel() {
        ...
        assert_eq!(map.get("clientInfo").map(String::as_str), Some("clientInfo"));
    }
    ...
    fn collect_args_renames_camel_to_snake() {
        ...
        let pairs = vec![
            ("sourceTimezone".to_string(), Value::String("Europe/Amsterdam".into())),
            ("targetTimezone".to_string(), Value::String("America/New_York".into())),
            ("time".to_string(), Value::String("15:00".into())),
        ];
        let out = collect_args_from_pairs(pairs, &map);
        let obj = out.as_object().unwrap();
        assert_eq!(obj.get("source_timezone").unwrap(), &json!("Europe/Amsterdam"));
        assert_eq!(obj.get("target_timezone").unwrap(), &json!("America/New_York"));
    ...
    fn collect_args_identity_for_already_camel_in_map() {
        ...
        let out = collect_args_from_pairs(pairs, &map);
        assert_eq!(out.as_object().unwrap().get("clientInfo").unwrap(), &json!("v"));
    }
    ...
    fn collect_args_unknown_key_falls_back_to_identity() {
        ...
        let map = HashMap::new();
        let pairs = vec![("__typename".to_string(), Value::String("ToolCallResult".into()))];
        let out = collect_args_from_pairs(pairs, &map);
        assert_eq!(out.as_object().unwrap().get("__typename").unwrap(), &json!("ToolCallResult"));
    }
    ...
    fn result_to_node_over_cap_error_mode() {
        ...
        assert!(text.contains("2000"), "must mention original size, got: {}", text);
        ...
        assert!(node.json.get("_data_prefix").is_none(), "error mode must NOT include data");
    }
    ...
    fn result_to_node_over_cap_truncate_mode() {
        ...
        assert_eq!(node.json["_original_bytes"], json!(2_000));
        assert_eq!(node.json["_cap_bytes"], json!(100));
        assert_eq!(node.json["_data_prefix"].as_str().map(|s| s.len()), Some(100));
    }
