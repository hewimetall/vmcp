...
**Язык:** [English](../builds-and-modes.md) | Русский
...
Локальный демо-стенд: [`demo/README.ru.md`](../../demo/README.ru.md).
...
<a id="optional-native-mcp-tasks-run_task"></a>
...
Server gate ≥96% (skills, prompt aggregation, tasks, sessions):
...
CI workflow [`ci.yml`](../../.github/workflows/ci.yml): пороги покрытия `vmcp-server` (≥96%), `vmcp-admin` (≥99%) и `vmcp-auth` (≥93%) проверяются при push в `main` / `cursor/**` и в pull requests.
